...
Firstly we load the data and encapsulate it in a Seurat object. Then we create
a new meta data by re-labelling the times 0 and 24 to 'Early' and 48 and 72
to 'Late'.
...
hsmm_hours <- as.character(FetchData(hsmm, vars.all = "Hours")[,1])
hsmm_hours[hsmm_hours == "0"] <- "Early"
hsmm_hours[hsmm_hours == "24"] <- "Early"
hsmm_hours[hsmm_hours == "48"] <- "Late"
hsmm_hours[hsmm_hours == "72"] <- "Late"
hsmm_hours <- as.factor(hsmm_hours)
hsmm <- SetIdent(hsmm, ident.use = hsmm_hours)
hsmm <- StashIdent(hsmm, save.name = "Condition")
```

Next we will find the highly variable genes for this data and use then to build
the PCA space. Finally, we cluster our data using the Seurat function
`FindClusters` and project the data onto t-SNE space for visualisation.

The `resolution` parameter of `FindCluster` was set from the default value (one)
to two, in order to increase the amount of clusters given by the algorithm. The
`perplexity` parameter was reduce from the default value of 30 to 15, as this
data does not have many data points.

```{r cluster, results='hide'}
hsmm <- FindVariableGenes(hsmm, do.plot = FALSE)
hsmm <- RunPCA(hsmm)

hsmm <- FindClusters(hsmm, reduction.type = "pca", dims.use = 1:5, resolution = 2)
hsmm <- StashIdent(hsmm, save.name = "Cluster")
hsmm <- RunTSNE(hsmm, reduction.use = "pca", dims.use = 1:5, do.fast = TRUE, perplexity = 15)
TSNEPlot(hsmm, group.by = "Condition", do.return = TRUE, pt.size = 0.5)
TSNEPlot(hsmm, do.return = TRUE, pt.size = 0.5, do.label = TRUE, label.size = 5)
```
...
Next we plot the results of the analysis. We can see that group 3, which has a
near 1:1 ratio between conditions, also has a high Z-score, reinforcing the
indication of high heterogeneity in those groups.
...
```{r condcomp_plot, fig.width=5, fig.height=5}
